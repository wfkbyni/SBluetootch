...
@synthesize aceDrawingView = _aceDrawingView;
...
- (void)_setupSubviews
{
    ...
    [self.faceButton setImage:[UIImage imageNamed:@"shock_off"] forState:UIControlStateNormal];
    ...
    EaseChatToolbarItem *faceItem = [[EaseChatToolbarItem alloc] initWithButton:self.faceButton withView:[self addDrawingView]];
    
    //更多
    self.moreButton = [[UIButton alloc] init];
    self.moreButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
    [self.moreButton setImage:[UIImage imageNamed:@"online_off"] forState:UIControlStateNormal];
 ...
 *  @brief 添加指画view
 */
- (UIView *)addDrawingView{
    
    if (_aceDrawingView == nil) {
        _aceDrawingView = [[ACEDrawingView alloc] initWithFrame:CGRectMake(0, ScreenSize.height - 200 - 100 - 30, ScreenSize.height, 216) withisHiddenBtn:YES];
        [_aceDrawingView setBackgroundColor:[UIColor orangeColor]];
        _aceDrawingView.lineWidth = 2;
        _aceDrawingView.lineColor = [UIColor greenColor];
        
    }
    
    [_aceDrawingView startSendDrawData:^(NSMutableArray *drawData) {
        ...
        for (int i = 0; i < array.count; i++) {
            [dataArray addObject:[array[i] toJSONString]];
        }
        
        UserMessage *receiveUser = [PublicMethod getReceiveUserMessage];
        [EaseSDKHelper sendImageMessageWithImage:_aceDrawingView.image to:receiveUser.Phone messageType:eMessageTypeChat requireEncryption:YES messageExt:@{@"data":dataArray} progress:NULL];
        
        [_aceDrawingView clear];
    }];
    
    return _aceDrawingView;
}

- (UIView *)moreView
{
    if (_moreView == nil) {
        _moreView = [[EaseChatBarMoreView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_toolbarView.frame), self.frame.size.width, 216) type:self.chatBarType];
        ...
        EaseChatBarMoreView *easeChatBarMoreView = (EaseChatBarMoreView *)_moreView;
        [easeChatBarMoreView.mySlider startSendSliderDrawData:^(NSString *drawData) {
            ...
            [business sendData:drawData receiveUserId:userMessage.AccountId];
        }];
    }
...
- (void)setDelegate:(id<EMChatToolbarDelegate>)delegate
{
    ...
    if ([_aceDrawingView isKindOfClass:[ACEDrawingView class]]) {
        [(ACEDrawingView *)_aceDrawingView setTheDelegate:delegate];
    }
}
...
- (void)setAceDrawingView:(ACEDrawingView *)aceDrawingView{
    if (_aceDrawingView != aceDrawingView) {
        _aceDrawingView = aceDrawingView;
        
        for (EaseChatToolbarItem *item in self.rightItems) {
            if (item.button == self.faceButton) {
                item.button2View = _aceDrawingView;
            }
        }
    }
}
