...
- (void)setupSubviewsForType:(EMChatToolbarType)type
{
    ...
    _maxIndex = 2;
    [_scrollview addSubview:_cacheDataButton];
    ...
    [_scrollview addSubview:_requestOnlineButton];    frame.size.height = 216;
    self.frame = frame;
    ...
    if (_mySlider == nil) {
        _mySlider = [[MySlider alloc] init];
        
        [_scrollview addSubview:_mySlider];
    }
}
...
{
    if(_delegate && [_delegate respondsToSelector:@selector(moreViewCacheDataAction:)]){
        [_delegate moreViewCacheDataAction:self];
    }
}

- (void)requestOnlineAction
{
    if (_delegate && [_delegate respondsToSelector:@selector(moreViewRequestOnlineAction:)]) {
        [_delegate moreViewRequestOnlineAction:self];
    }
}
