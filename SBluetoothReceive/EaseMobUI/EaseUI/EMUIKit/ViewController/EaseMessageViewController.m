...
- (void)viewDidLoad {
    ...
    self.aceDrawingView = (ACEDrawingView*)[(EaseChatToolbar *)self.chatToolbar aceDrawingView];
...
- (void)cacheData{
    ...
    [SVProgressHUD showErrorWithStatus:@"开始缓存请稍候"];
 ...
 */
- (void)requestOnline{
    UserMessage *loginMessage = [PublicMethod getLoginUserMessage];
    
    NSDictionary *param = @{@"AccountId" : loginMessage.AccountId};
    
    [[HttpClientHelper sharedInstance] post:FriendsWithCanPlay resultType:[BaseResponseModel class] parameters:param success:^(BaseResponseModel *result) {
        
        if (result.Success) {
            UserMessage *reveiceMessage = [PublicMethod getReceiveUserMessage];
            [appDelegate connectionSocketAndSendData:SocketCommandRequestMachineConnect receiveUserId:reveiceMessage.AccountId data:@"020001"];
        }else{
...
#pragma AcedDrawingViewDelegate
-(void)acedDrawingViewCacheDataAction:(ACEDrawingView *)moreView{
    [self cacheData];
}

- (void)acedDrawingViewRequestOnlineAction:(ACEDrawingView *)moreView{
    [self requestOnline];
}

#pragma EaseChatBarMoreViewDelegate
- (void)moreViewCacheDataAction:(EaseChatBarMoreView *)moreView
{
    [self cacheData];
}

- (void)moreViewRequestOnlineAction:(EaseChatBarMoreView *)moreView
{
    // 请求联机
    [self requestOnline];
}
...
-(void)didReceiveMessage:(EMMessage *)message
{
    ...
    if (message.ext != nil) {
        NSArray *array = [message.ext valueForKey:@"data"];
        
        NSMutableArray *dataArray = [NSMutableArray arrayWithCapacity:array.count];
        NSError *error;
        for (int i = 0; i < array.count; i++) {
            
            CacheDataInfo *info = [[CacheDataInfo alloc] initWithString:array[i] error:&error];
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            @autoreleasepool {
                ...
                for (CacheDataInfo *info in dataArray) {
...
- (void)sendTextMessage:(NSString *)text withExt:(NSDictionary*)ext
{
    ...
    ext = @{@"headerUrl":[userMessage getStrPicPath],
