...
-(instancetype)init{
    ...
    if (self) {
        ...
        self.allServers = [NSMutableArray new];
...
- (void)writeData:(NSArray *)dataList{
    ...
    for (CacheDataInfo *obj in dataList) {
        @autoreleasepool {
            ...
            if (currCharacteristic == nil) {
                currCharacteristic = [BabyToy findCharacteristicFormServices:self.allServers UUIDString:WriteDateUUID];
            }
            ...
            //[self.currPeripheral readValueForCharacteristic:currCharacteristic];
        }
    }
