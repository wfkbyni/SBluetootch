...
- (void)viewDidLoad {
    ...
    NSArray *array = [[NSArray alloc]initWithObjects:
                      ...
                      [UIImage imageNamed:@"chat_sender_audio_playing_003"], nil];
...
- (void)backAction:(id)sender{
    ...
    if ([[[NSUserDefaults standardUserDefaults] valueForKey:IsConnectioning] boolValue]) {
        
        NSString *message = @"您确定要退出联机吗？";
        SocketCommandEnum command = SocketCommandRequestStopMachineConnect;
        // 请求退出次数
        int quitCount = [[[NSUserDefaults standardUserDefaults] valueForKey:RequestQuitCount] intValue];
        if (quitCount > 3) {
            message = @"您确定要强制退出联机吗？";
            command = SocketCommandMustStopMachineConnect;
        }
        ...
        [UIAlertView alertWithTitles:@"提示" message:message clickBlock:^(UIAlertView *alertView, NSInteger clickIndex) {
            ...
            if (clickIndex != alertView.cancelButtonIndex) {
                ...
                [appDelegate connectionSocketAndSendData:command receiveUserId:_receiverUser.AccountId data:@""];
...
- (UITableViewCell *)messageViewController:(UITableView *)tableView cellForMessageModel:(id<IMessageModel>)model
{
    if (model.bodyType == eMessageBodyType_Text ||
        ...
        if (model.isSender) {
            ...
            UserMessage *userInfo = [PublicMethod getLoginUserMessage];
            
            model.avatarURLPath = [userInfo getStrPicPath];
            model.nickname = userInfo.NickName;
        }else{
            model.avatarURLPath = [self.receiverUser getStrPicPath];
            model.nickname = self.receiverUser.NickName;
        }
...
- (void)messageViewController:(EaseMessageViewController *)viewController
   didSelectAvatarMessageModel:(id<IMessageModel>)messageModel{
    FriendMessageController *controller = [[FriendMessageController alloc] init];
    controller.userMessage = self.receiverUser;
